...
function AgentCard({ icon: Icon, label, x, y, delay, size = 'md', floatDuration = 5, floatDistance = 8 }: AgentCardProps) {
  const sizes = {
    sm: { card: 'w-12 h-12', icon: 16, text: 'text-[9px]' },
    md: { card: 'w-14 h-14', icon: 22, text: 'text-[10px]' },
    lg: { card: 'w-[72px] h-[72px]', icon: 28, text: 'text-xs' },
  };
  const s = sizes[size];
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 w-full grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        ...
        <div className="order-2 lg:order-1">
          ...
          <motion.p
            ...
            className="text-lg text-text-dim max-w-lg mb-8 leading-relaxed"
          >
            {t('hero.subtitle')}
          </motion.p>
          ...
          >
            <motion.a
              ...
              whileTap={{ scale: 0.97 }}
              className="inline-flex items-center justify-center gap-2 px-7 py-3.5 bg-primary hover:bg-primary/90 text-white font-semibold rounded-full shadow-glow transition-colors"
            >
              {t('hero.cta')}
              <ArrowRight size={18} />
            </motion.a>
            <motion.a
              ...
              whileHover={{ scale: 1.03 }}
              whileTap={{ scale: 0.97 }}
              className="inline-flex items-center justify-center px-7 py-3.5 border border-white/20 text-white font-semibold rounded-full hover:bg-white/5 transition-colors"
            >
              {t('hero.secondary')}
            </motion.a>
          </motion.div>
        ...
        <motion.div
          ...
          className="order-1 lg:order-2 relative h-[350px] sm:h-[400px] lg:h-[500px]"
        >
          {/* Ambient glow blobs */}
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-primary/15 rounded-full blur-[80px]" />
          <div className="absolute top-1/3 left-1/3 w-32 h-32 bg-accent/10 rounded-full blur-[60px]" />
          ...
          {/* Center - Orchestrator (larger) */}
          <AgentCard
            icon={Network} label={t('hero.nodeMain')}
            x="50%" y="45%"
            delay={0.5} size="lg"
            floatDuration={6} floatDistance={6}
          />

          {/* Surrounding agents */}
          <AgentCard
            icon={Bot} label={t('hero.nodeAgent1')}
            x="20%" y="18%"
            delay={0.7} size="md"
            floatDuration={5} floatDistance={10}
          />
          <AgentCard
            icon={Cpu} label={t('hero.nodeAgent2')}
            x="78%" y="15%"
            delay={0.85} size="md"
            floatDuration={4.5} floatDistance={9}
          />
          <AgentCard
            icon={Database} label={t('hero.nodeAgent3')}
            x="15%" y="70%"
            delay={1.0} size="md"
            floatDuration={5.5} floatDistance={8}
          />
          <AgentCard
            icon={Workflow} label={t('hero.nodeAgent4')}
            x="82%" y="65%"
            delay={1.15} size="md"
            floatDuration={4.8} floatDistance={11}
          />
          <AgentCard
            icon={Bot} label={t('hero.nodeAgent5')}
            x="48%" y="85%"
            delay={1.3} size="sm"
            floatDuration={5.2} floatDistance={7}
          />
        </motion.div>
      </div>
